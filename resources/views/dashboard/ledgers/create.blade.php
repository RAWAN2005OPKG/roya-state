...
@section('content')
<form action="{{ route($routeName.'.store') }}" method="POST" id="voucher-form">
    @csrf
    @if ($errors->any())<div class="alert alert-danger"><ul>@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>@endif

    {{-- الكود هنا مطابق تماماً لملف إنشاء السندات الأصلي الذي أرسلته لك سابقاً --}}
    {{-- لا حاجة لتغيير أي شيء في محتوى النموذج، فقط مسار الإرسال في الأعلى --}}
    ...
    <div class="card mb-5"><div class="card-body">
        ...
        <div class="row">
            <div class="col-md-4 form-group"><label>المبلغ <span class="text-danger">*</span></label><input type="number" name="amount" id="amount" class="form-control" value="{{ old('amount') }}" step="0.01" required></div>
            ...
            <div class="col-md-2 form-group" id="exchange_rate_wrapper"><label>سعر الصرف</label><input type="number" name="exchange_rate" id="exchange_rate" class="form-control" value="{{ old('exchange_rate', 3.75) }}" step="0.001"></div>
    ...
    <div class="card mb-5"><div class="card-body">
        ...
        <div class="row">
            <div class="col-md-4 form-group"><label>ربط بمشروع</label><select name="project_id" class="form-control select2-basic"><option value="">-- اختر مشروع --</option>@foreach($projects as $project)<option value="{{ $project->id }}" @selected(old('project_id') == $project->id)>{{ $project->name }}</option>@endforeach</select></div>
            <div class="col-md-8 form-group"><label>ملاحظات</label><textarea name="notes" class="form-control">{{ old('notes') }}</textarea></div>
        </div>
    </div></div>
...
$(document ).ready(function() {
    ...
    $('#currency').on('change', function() {
        ...
        if (this.value === 'ILS') $('#exchange_rate').val(1);
        calculateILS();
    });
