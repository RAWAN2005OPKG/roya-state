...
<main class="main-content">
    <div class="card card-custom" style="max-width: 900px; margin: auto;">
        <div class="card-header"><h3 class="card-title">إضافة فاتورة شراء</h3></div>
        <div class="card-body">
            @if ($errors->any())
                <div class="alert alert-danger"><ul>@foreach ($errors->all() as $error) <li>{{ $error }}</li> @endforeach</ul></div>
            @endif

            <form action="{{ route('dashboard.purchases.invoices.store') }}" method="POST">
                @csrf
                <div class="row">
                ...
                <div id="items-container">
                    {{-- سيتم إضافة بنود الفاتورة هنا عبر JavaScript --}}
                    <div class="row item-row mb-3">
                        <div class="col-md-4"><label>المنتج</label><select name="items[0][product_id]" class="form-control product-select"><option value="">اختر منتج</option></select></div>
                        <div class="col-md-2"><label>الكمية</label><input type="number" name="items[0][quantity]" class="form-control quantity-input" value="1" min="1"></div>
                        <div class="col-md-3"><label>سعر الوحدة</label><input type="number" name="items[0][unit_price]" class="form-control price-input" value="0.00" step="0.01"></div>
                        <div class="col-md-2"><label>الإجمالي</label><input type="text" class="form-control total-output" value="0.00" readonly></div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 form-group mb-3">
                        ...
                        <input type="number" name="paid_amount" id="paid_amount" class="form-control" value="{{ old('paid_amount', 0) }}" step="0.01" required>
                    </div>
                    <div class="col-md-6 form-group mb-3">
                        <label>المتبقي:</label>
                        <input type="text" id="remaining-amount-output" class="form-control" value="0.00" readonly>
                    </div>
                </div>
