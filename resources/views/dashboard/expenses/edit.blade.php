...
<main class="main-content">
    ...
    <div class="form-container">
        ...
        <form id="expenseForm" class="form-grid" action="{{ route('dashboard.expenses.update', $expense->id) }}" method="POST">
            ...
            <div class="form-group"><label for="date">تاريخ الدفع</label><input type="date" name="date" required value="{{ old('date', $expense->date->format('Y-m-d')) }}"></div>
            <div class="form-group"><label for="payee">اسم المستفيد</label><input type="text" name="payee" required value="{{ old('payee', $expense->payee) }}"></div>
            <div class="form-group">
                <label for="project_id">المشروع</label>
                <select name="project_id">
                    <option value="">-- اختر المشروع (اختياري) --</option>
                    <option value="0" @selected(old('project_id', $expense->project_id) === null)>مصروف عام (لا يتبع لمشروع)</option>
            ...
            <div class="form-group"><label for="amount">المبلغ</label><input type="number" id="amount" name="amount" min="0" step="0.01" required value="{{ old('amount', $expense->amount) }}"></div>
            <div class="form-group">
                <label for="currency">العملة</label>
                <select id="currency" name="currency" required>
                    <option value="ILS" @selected(old('currency', $expense->currency) == 'ILS')>شيكل (ILS)</option>
                    <option value="USD" @selected(old('currency', $expense->currency) == 'USD')>دولار (USD)</option>
                    <option value="JOD" @selected(old('currency', $expense->currency) == 'JOD')>دينار (JOD)</option>
                </select>
            </div>
            <div id="exchangeRateSection" class="form-group" style="display: none;">
                <label for="exchange_rate">سعر الصرف مقابل الشيكل</label>
                <input type="number" id="exchange_rate" name="exchange_rate" step="0.01" value="{{ old('exchange_rate', $expense->exchange_rate) }}">
            ...
            <div class="form-group" style="grid-column: 1 / -1;"><label for="details">تفاصيل المصروف</label><textarea name="details" rows="3">{{ old('details', $expense->details) }}</textarea></div>
...
@push('scripts')
{{-- نفس السكريبت الموجود في ملف create.blade.php --}}
<script>
