...
<form action="{{ route('dashboard.checks.store') }}" method="POST">
    ...
    <div class="card card-custom">
        <div class="card-header">
            <h3 class="card-title">إدخال بيانات الشيك الجديد</h3>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <select name="type" class="form-control" required>
                        <option value="receivable" {{ old('type') == 'receivable' ? 'selected' : '' }}>شيك قبض (وارد)</option>
                        <option value="payable" {{ old('type') == 'payable' ? 'selected' : '' }}>شيك دفع (صادر)</option>
                    </select>
                </div>
                <div class="col-md-6 form-group">
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <input type="text" name="party_phone" class="form-control" value="{{ old('party_phone') }}">
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-2 form-group" id="exchange_rate_wrapper">
                    ...
                    <input type="number" name="exchange_rate" id="exchange_rate" class="form-control" value="{{ old('exchange_rate', 1) }}" step="0.001">
                </div>
                <div class="col-md-3 form-group">
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <select name="deposit_bank_account_id" class="form-control select2-basic">
                        ...
                        @foreach($bankAccounts as $account)
                            <option value="{{ $account->id }}" {{ old('deposit_bank_account_id') == $account->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 form-group">
                    ...
                    <select name="payment_bank_account_id" class="form-control select2-basic">
                        <option value="">-- اختر حساب --</option>
                        @foreach($bankAccounts as $account)
                            <option value="{{ $account->id }}" {{ old('payment_bank_account_id') == $account->id ? 'selected' : '' }}>
                                {{ $account->account_name }} ({{$account->bank->name}})
                            </option>
                        @endforeach
                    </select>
...
$(document).ready(function() {
    ...
    $('#currency').on('change', function() {
        if (this.value === 'ILS') {
            ...
            if ($('#exchange_rate').val() == 1) {
                // قيم افتراضية تقريبية
                if (this.value === 'USD') $('#exchange_rate').val(3.75);
                if (this.value === 'JOD') $('#exchange_rate').val(5.25);
            }
        }
        calculateILS();
    });
