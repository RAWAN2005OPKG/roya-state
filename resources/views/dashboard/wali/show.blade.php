...
@section('title', 'عرض سند وليد رقم ' . $voucher->id)

@push('styles')
<style>
    .details-card { border: 1px solid #eee; border-radius: 8px; }
    .details-card .card-header { background-color: #f7f7f7; }
    .details-card .list-group-item { border-left: 0; border-right: 0; padding: 1rem 1.5rem; }
    .details-card .list-group-item strong { min-width: 150px; display: inline-block; color: #555; }
</style>
@endpush
...
<div class="card card-custom gutter-b">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-receipt text-warning mr-2"></i>تفاصيل السند رقم: {{ $voucher->id }}</h3>
        <div class="card-toolbar">
            <a href="#" class="btn btn-light-primary font-weight-bold mr-2" onclick="window.print();"><i class="fas fa-print"></i> طباعة</a>
            <a href="{{ route('dashboard.wali.index') }}" class="btn btn-secondary">العودة إلى القائمة</a>
        </div>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-6"><div class="card details-card mb-5"><div class="card-header"><h5 class="card-title m-0">المعلومات الأساسية</h5></div><ul class="list-group list-group-flush">
                ...
                <li class="list-group-item"><strong>نوع السند:</strong> @if($voucher->type == 'receipt')<span class="badge badge-success">سند قبض</span>@else<span class="badge badge-danger">سند صرف</span>@endif</li>
            ...
            <div class="col-md-6"><div class="card details-card mb-5"><div class="card-header"><h5 class="card-title m-0">المعلومات المالية</h5></div><ul class="list-group list-group-flush">
        ...
        <div class="card details-card mb-5"><div class="card-header"><h5 class="card-title m-0">تفاصيل طريقة الدفع</h5></div><div class="card-body">
            ...
            @elseif($voucher->payment_method == 'check' && $voucher->details)
                <p><strong>رقم الشيك:</strong> {{ $voucher->details->check_number }}</p><p><strong>اسم صاحب الشيك:</strong> {{ $voucher->details->check_owner_name }}</p><p><strong>اسم البنك:</strong> {{ $voucher->details->check_bank_name }}</p><p><strong>تاريخ الاستحقاق:</strong> {{ \Carbon\Carbon::parse($voucher->details->check_due_date)->format('d-m-Y') }}</p>
            @elseif($voucher->payment_method == 'bank_transfer' && $voucher->details)
                <p><strong>من حساب بنكي:</strong> {{ $voucher->details->fromBankAccount->account_name ?? '-' }}</p><p><strong>إلى حساب بنكي:</strong> {{ $voucher->details->toBankAccount->account_name ?? '-' }}</p>
            @else
                <p class="text-muted">لا توجد تفاصيل إضافية لطريقة الدفع هذه.</p>
            @endif
        </div></div>
        <div class="card details-card"><div class="card-header"><h5 class="card-title m-0">معلومات الربط</h5></div><ul class="list-group list-group-flush">
